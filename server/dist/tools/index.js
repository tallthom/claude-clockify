...
export class ClockifyTools {
    ...
    isProtectedProject(projectId) {
        ...
        if (defaultProjectId && projectId === defaultProjectId) {
            return {
                isProtected: true,
                reason: 'This is the configured default project. Please update your configuration to remove the default project setting first.',
            };
        }
        ...
        if (restrictions.allowedProjects &&
            ...
            return {
                isProtected: true,
                reason: 'This is the only allowed project in the configuration. Please update your allowedProjects configuration first.',
            };
        }
        return { isProtected: false };
    }
    isConfiguredProject(projectId) {
        const defaultProjectId = this.config.getDefaultProjectId();
        const restrictions = this.config.getRestrictions();
        return ((defaultProjectId !== undefined && projectId === defaultProjectId) ||
        ...
        return [
                ...
                handler: async (input) => {
                    ...
                    if (defaultProjectId || restrictions.allowedProjects) {
                        ...
                        if (input.settings?.onlyAdminsCreateProject === true) {
                            throw new Error('Cannot enable "only admins create project" setting when projects are configured. This could prevent access to project functionality.');
                        }
                        if (input.settings?.lockTimeEntries) {
                            console.warn('Warning: Changing time entry locking policy with configured projects. This may affect existing time tracking workflows.');
                        }
                        // General warning for any settings change
                        console.warn('Warning: Updating workspace settings with configured projects. Ensure these changes do not affect required project functionality.');
                    }
                ...
                handler: async (input) => {
                    ...
                    if (this.isConfiguredProject(projectId)) {
                        ...
                        if (data.archived === true) {
                            throw new Error('Cannot archive a configured project through update. Please update your configuration first.');
                        }
                        ...
                        if (data.name || data.clientId !== undefined) {
                            console.warn(`Warning: Updating critical fields of configured project ${projectId}. Ensure this change is intentional.`);
                        }
                    }
            ...
            },
            {
                ...
                handler: async (input) => {
                    ...
                    const result = await this.projectService.addUserToProject(input.workspaceId, input.projectId, input.userId, input.hourlyRate ? { hourlyRate: input.hourlyRate } : undefined);
            ...
            },
            {
                ...
                handler: async (input) => {
                    ...
                    if (this.isConfiguredProject(input.projectId)) {
                        throw new Error('Cannot remove users from a configured project. This could break access to required project functionality.');
                    }
                    await this.projectService.removeUserFromProject(input.workspaceId, input.projectId, input.userId);
            ...
            },
            {
                ...
                handler: async (input) => {
                    ...
                    if (defaultProjectId || restrictions.allowedProjects) {
                        ...
                        try {
                            const projects = await this.projectService.getProjectsForClient(input.workspaceId, input.clientId);
                            ...
                            if (configuredProjects.length > 0) {
                                ...
                                throw new Error(`Cannot delete client: The following configured projects depend on this client: ${projectNames}. Please update your project configuration first.`);
                            }
                        }
                        catch (error) {
                            // If we can't check, err on the side of caution
                            if (error instanceof Error && !error.message.includes('Cannot delete client')) {
                                console.warn('Warning: Unable to verify client dependencies for configured projects. Proceeding with deletion.');
                            }
                            else {
                                throw error;
                            }
            ...
            },
            {
                ...
                handler: async (input) => {
                    ...
                    if (!data.projectId) {
                        try {
                            const existing = await this.timeEntryService.getTimeEntryById(workspaceId, timeEntryId);
            ...
            },
            {
                ...
                handler: async (input) => {
                    const entry = await this.timeEntryService.duplicateTimeEntry(input.workspaceId, input.timeEntryId);
                    return { success: true, data: entry };
                },
            },
            {
            ...
            },
            {
                ...
                handler: async (input) => {
                    ...
                    if (action === 'UPDATE' && updates?.projectId) {
                        ...
                        if (defaultProjectId || restrictions.allowedProjects) {
                            console.warn('Warning: Bulk operation is changing project assignments. Ensure this does not affect configured project requirements.');
                        }
                        // Don't allow moving TO a configured project in bulk (could overwhelm it)
                        if (this.isConfiguredProject(updates.projectId)) {
                            throw new Error('Cannot bulk assign time entries to a configured project. Please assign individually to avoid overwhelming the configured project.');
                        }
                    }
                    if (action === 'DELETE') {
            ...
            },
            {
                ...
                handler: async (input) => {
                    ...
                    const defaultProjectId = this.config.getDefaultProjectId();
                    if (defaultProjectId || restrictions.allowedProjects) {
                        console.warn('Warning: Deleting tag from workspace with configured projects. Ensure this tag is not required for project workflows.');
                    }
            ...
            },
            {
                ...
                handler: async (input) => {
                    ...
                    if (this.isConfiguredProject(input.projectId)) {
                        throw new Error('Cannot delete tasks from a configured project. Tasks in configured projects may be essential for required workflows.');
                    }
            ...
            },
            {
                ...
                handler: async (input) => {
                    ...
                    const customField = await this.customFieldService.updateCustomField(workspaceId, customFieldId, data);
                    return { success: true, data: customField };
                },
            },
            {
            ...
            },
            {
                ...
                handler: async (input) => {
                    const customField = await this.customFieldService.getCustomFieldById(input.workspaceId, input.customFieldId);
                    return { success: true, data: customField };
                },
            },
        ];

...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (error.message.includes('Invalid API key') || error.message.includes('unauthorized')) {
            throw new McpError(ErrorCode.InvalidRequest, 'Invalid Clockify API key. Please check your CLOCKIFY_API_KEY environment variable.');
        }
        else if (error.message.includes('Rate limit') || error.message.includes('429')) {
            throw new McpError(ErrorCode.InvalidRequest, 'Clockify API rate limit exceeded. Please try again later.');
        }
